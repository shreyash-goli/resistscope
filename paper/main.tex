...
\textbf{Limitations.} Rigid single-structure docking (no ensemble, flexible receptor, or explicit flap water), which bounds every claim we make about docking energy; crystal waters stripped; Vina score treated as a ranking signal, not a free energy; the RT subpocket map is a coarse approximation; RT mechanism ground truth is agent-built and needs a human-expert anchor; the LLM judge's absolute scores are judge-dependent (cross-vendor $\kappa\approx0.23$--0.38 on hard cases, cross-Claude-model $\kappa\approx0.64$--0.72) though the paired contrasts reproduce across judges; per-bin $n$ is modest and many tests are reported without multiplicity correction; and compound-level scoring aggregation is underpowered (6 $+$ 4 compounds), so the deployed 0--100 robustness score is itself unvalidated and all quantitative validation is per-mutation.
